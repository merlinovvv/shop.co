...
export const AuthModal = () => {
  const { isOpenModal, setIsOpenModal, isLoginProccess, setIsLoginProccess } = useAuthStore();
  return (
          ...
          <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
            ...
            >
              ...
              <Description className="text-gray-500">
                Please <button onClick={() => setIsLoginProccess(true)} className={clsx({ "cursor-pointer underline": !isLoginProccess })}>log in</button> or{" "}
                <button onClick={() => setIsLoginProccess(false)} className={clsx({ "cursor-pointer underline": isLoginProccess })}>register</button> to continue
              </Description>
              {isLoginProccess ? <LoginForm/> : <RegisterForm/>}
            </DialogPanel>
          </div>
        </div>
      </div>
